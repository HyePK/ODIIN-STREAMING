...
export function CreatorStudio({ email }: { email: string }) {
  ...
  useEffect(() => {
    void load();
    return () => { mediaRef.current?.getTracks().forEach((track) => track.stop()); peerRef.current?.close(); };
  }, []);
  ...
  if (!busy && !creator) return <main className="studio-denied"><a href="/"><ChevronLeft /> ODIIN STREAMING</a><LockKeyhole /><h1>Creator subscription required</h1><p>{message}</p><a className="studio-account-link" href="/account">View your ODIIN account</a></main>;

  return <main className="studio-page">
    <header><a href="/"><ChevronLeft /> Viewer site</a><span><img src="/odiin-mark.svg" alt="" /><b>ODIIN</b> CREATOR STUDIO</span><em>{creator?.subscriptionPlan || "Creator"}</em></header>
