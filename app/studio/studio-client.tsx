  ...
  async function createBroadcast(event: FormEvent) {
    ...
    try {
      ...
      if (mode === "recorded" && recording) {
        setUploadProgress(0);
        await uploadVideoInChunks(data.event.id, recording, { onProgress: setUploadProgress });
        setRecording(null); setUploadProgress(null); setMessage("Recorded event published, verified in permanent storage, and ready to play.");
      ...
      await load();
    } catch (error) { setUploadProgress(null); setMessage(`${error instanceof Error ? error.message : "Unable to create broadcast."} The failed record remains in the dashboard for retry.`); await load().catch(() => undefined); }
    finally { setBusy(false); }
  }
