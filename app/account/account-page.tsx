...
export function AccountPage({ email, defaultName }: { email: string; defaultName: string }) {
  ...
  return <main className="account-page"><Link className="account-back" href="/"><ChevronLeft /> Back to ODIIN STREAMING</Link><section className="account-card"><span className="account-icon">{exists ? <Check /> : <UserRound />}</span><p className="eyebrow">ODIIN account</p><h1>{exists ? "Your viewer profile" : "Create your account"}</h1><p>Save broadcasts to your library and return to them from any signed-in device.</p><form onSubmit={submit}><label>Display name<Input value={name} onChange={(e) => setName(e.target.value)} required /></label><label>Email<Input value={email} disabled /></label><Button className="primary-action" disabled={busy}>{busy && <LoaderCircle className="spin" />}{exists ? "Update account" : "Create ODIIN account"}</Button></form>{message && <div className="account-message">{message}</div>}{exists && <div className="account-stats"><Heart fill="currentColor" /><b>{savedCount}</b><span>Saved videos</span></div>}{creatorActive && <Link className="creator-studio-link" href="/studio">Open ODIIN Creator Studio</Link>}</section></main>;
}
